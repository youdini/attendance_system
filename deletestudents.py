...
from tkinter import Tk, Canvas, Entry, Button, PhotoImage, ttk, Frame, BOTH, Scrollbar, END, messagebox, StringVar
...
class DeleteStudent:
    ...
    def selected_item(self, event):
        ...
        if len(values) > 0:
            ...
            if response:
                push_to_archive = (f"""
                        INSERT INTO archive (id, firstname, lastname, course, section, student_id, course_code, 
                        time, day, lab_room) SELECT * FROM students WHERE id = '{fetch_id[0]}'
                        """)
                cursor.execute(push_to_archive)
                db.commit()
                delete_student = f"""DELETE FROM students WHERE id = '{fetch_id[0]}'"""
    ...
    def archive_window(self):
        self.master.destroy()
        subprocess.run(["python", "./archivewindow.py"])
